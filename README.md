...
- 💬 **我的评论** — 查看自己的评论，并直接发表商品评论
- 🗨️ **咨询聊天** — 按商户发起咨询，查看会话历史并发送消息
- ⚙️ **个人设置** — 查看账户信息
- 🔌 **开放接口** — 在门户中申请 API Token、查看/撤销 Token、创建/查看/删除 Webhook
  - Token 支持 **无期限** / **有期限**
  - Webhook 支持 **无期限** / **有期限**
...
- `POST /comments` — 创建评论
- `GET /comments` — 当前用户自己的评论列表
- `GET /comments/merchant/me` — 当前商户收到的评论列表
- `GET /comments/product/{id}` — 商品评论列表
- `POST /chat/messages` — 消费者向商户发送消息
- `GET /chat/messages/{merchant_id}` — 消费者查看与某商户的聊天记录
- `GET /chat/merchant/conversations` — 商户侧会话列表
- `GET /chat/merchant/messages/{user_id}` — 商户查看与某用户的聊天记录
- `POST /chat/merchant/messages/{user_id}` — 商户回复用户消息
...
- 增加商户门户“评论管理 / 客户聊天”入口与页面
- 增加消费者门户“我的评论 / 咨询聊天”入口与页面
- 增加评论快捷入口：消费者可从商品浏览页直接进入“评价”
- 增加聊天快捷入口：消费者可从商品/商户浏览页直接进入“咨询”
- 修复聊天消息缺少发送方标识的问题，新增 `sender_role`
- 为现有 SQLite 数据库自动补充 `chat_messages.sender_role` 字段，避免手工迁移
- 修复消费者聊天记录接口中 `order_by` 调用顺序错误导致的 500
- 打开聊天会话时自动更新已读状态，消费者和商户两侧都可正确消除未读数
...
- 商户成功回复用户消息
- 消费者成功查看完整聊天记录
- 商户门户已包含“评论管理 / 客户聊天”入口和对应页面容器
- 消费者门户已包含“我的评论 / 咨询聊天”入口和对应页面容器
